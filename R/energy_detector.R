    ...
    detect_FUN <-
               ...
               )
      {
        ...
        hop.samples <- envlp$duration / (length(envlp$envelope) - 1)
          ...
          if (peak.amplitude > 0){
            detections_df <- warbleR::sound_pressure_level(detections_df, parallel = 1, path = path, pb =  FALSE, peak.amplitude = TRUE)

            detections_df <- detections_df[detections_df$peak.amplitude > peak.amplitude, ]

            # remove extra column
            detections_df$peak.amplitude <- NULL
            }
