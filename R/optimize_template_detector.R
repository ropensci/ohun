...
optimize_template_detector <- function(template.correlations, reference, threshold, cores = 1, pb = TRUE, by.sound.file = FALSE, previous.output = NULL){

  if (!is(template.correlations, "template_correlations"))
    stop2("'template.correlations' must be an object of class 'template_correlations' generated by template_detector()")
  ...
  if (length(threshold) == 0){
    ...
    diagnostics_list <- warbleR:::pblapply_wrblr_int(pbar = pb, cl = 1, X = threshold, FUN = function(x){
      ...
      detection <- as.data.frame(template_detector(template.correlations = template.correlations, threshold = x,cores =  cores, pb = FALSE, verbose = FALSE))
      ...
      sub_diagnostics_list <- lapply(X = unique(templates), function(r){
        ...
        Q <- diagnose_detection(reference = reference, detection = detection[detection$template == r, ], by.sound.file = by.sound.file, cores = cores, pb = FALSE)
