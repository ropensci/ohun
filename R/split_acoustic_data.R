...
split_acoustic_data <- function(path = ".", sgmt.dur = 10, sgmts = NULL, files = NULL, cores = 1, pb = TRUE, only.sels = FALSE, X = NULL){
  ...
  if (!is.null(X)){
    ...
    if (!any(is.data.frame(X), warbleR::is_selection_table(X))) stop2("X is not of a class 'data.frame' or 'selection_table'")

    if (warbleR::is_extended_selection_table(X)) stop2("This function cannot take extended selection tables ('X' argument)")
    ...
    if (any(!methods::is(X$end, "numeric"), !methods::is(X$start, "numeric"))) stop2("'start' and 'end' must be numeric")
