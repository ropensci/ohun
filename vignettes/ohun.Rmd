...
Several additional indices derived from these indices can be used to evaluate the performance of a detection routine. These are two of the most widely used:
...
A perfect detection will have no false positives or false negatives, which will result in both sensitivity and specificity equal to 1. However, perfect detection cannot always be reached and some compromise between detecting all target signals plus some noise (sensitivity = 1 & specificity < 1) and detecting only target signals but not all of them (sensitivity < 1 & specificity = 1) is warranted. The right balance between these two options will be given by the relative costs of missing signals and mistaking noise for signals. Therefore, these indices can be used for diagnosing and optimizing the performance of a detection routine. 

The package [ohun](https://github.com/maRce10/ohun) provides a set of tools to evaluate the performance of an acoustic signal detection based on the indices 
described above.


```{r, eval = FALSE}

data(c("lbh_1", "lbh_reference"))

seewave::spectro(lbh_1, wl = 200, ovlp = 50, scale = FALSE, flim = c(1, 11), collevels = seq(-100, 0, 5), palette = reverse.gray.colors.1, grid = FALSE)

for(i in which(lbh_reference$sound.files == "lbh_1.wav"))
  lines(x = (lbh_reference[i, c("start", "end")]), y = c(2, 2), col = "#07889BCC", lwd = 6)
...
This detector uses amplitude envelopes to infer the position of signals. Amplitude envelopes are representations of the variation in energy through time. For instance we can calculate and plot an amplitude envelope from the example wave object `Phae.long1`:

```{r}
env <- warbleR::envelope(Phae.long1@left)
```

... and plot it along the spectrogram:
...
# plot spectrogram
seewave::spectro(Phae.long2, ovlp = 90, grid = FALSE, scale = FALSE, collevels = seq(-100, 0, 5), wl = 300, palette = reverse.gray.colors.1, oma = c(0, 0, 0 , 0), axisX = FALSE)

par(mar = c(4,  4,  0,  0))

# plot envelope
seewave::env(Phae.long2)
...
It doesn't require highly stereotyped signals, although they work better on high quality recordings in which the amplitude of target signals is higher than the background noise (i.e. high signal-to-noise ratio). The function `ernergy_detector()` performs this type of detection.  The syntax `package.name::function.name()` will be used here and throughout the vignette to highlight functions coming from packages other than [ohun](https://github.com/maRce10/ohun),
