...
# align figures
opts_chunk$set(fig.align='center', tidy = TRUE, fig.height=4.5, fig.width=10) 
```
&nbsp;
...
- **Specificity**: proportion of detected items that correspond to target signals (*(TPs + FNs) / (TPs + FPs)*). This definition is sometimes refer to as false positive rate.
...
# print spectrogram
label_spectro(wave = lbh1, reference = lbh_reference[lbh_reference$sound.files == "lbh1.wav", ], wl = 200, ovlp = 50, flim = c(1, 10))
...
# print spectrogram
label_spectro(wave = lbh1, reference = lbh1_reference, detection = lbh1_detection, wl = 200, ovlp = 50, flim = c(1, 10))
...
# print spectrogram
label_spectro(wave = lbh1, detection = lbh1_reference, reference = lbh1_detection, wl = 200, ovlp = 50, flim = c(1, 10))
...
label_spectro(wave = cutw(lbh1, from = 0, to = 1.5, output = "Wave"), ovlp = 90, wl = 300, flim = c(0, 10), envelope = TRUE)
...
# run detection
detection <- energy_detector(files = "simulated_1.wav", bp = c(2, 8), threshold = 0.5, smooth = 150, path = tempdir())

# plot spectrogram and envelope
label_spectro(wave = simulated_1, envelope = TRUE,  detection = detection, threshold = 0.5)

```
&nbsp;

The output is a  'selection_table'object (*sensu* [warbleR](https://CRAN.R-project.org/package=warbleR)):
...
# run detection
detection <- energy_detector(files = "simulated_1.wav", bp = c(1, 8), threshold = 0.5, min.duration = 500, smooth = 150, path = tempdir())
...
# run detection
detection <- energy_detector(files = "simulated_1.wav", bp = c(1, 8),  threshold = 0.5, smooth = 150, max.duration = 500, path = tempdir())
...
# Detecting 
detection <- energy_detector(files = "simulated_1.wav", bp = c(5, 8),  threshold = 0.5, smooth = 150, path = tempdir())
...
# Detect
detection <- energy_detector(files = "simulated_1.wav", bp = c(0, 6),  threshold = 0.5, min.duration = 1, smooth = 150, path = tempdir())
...
# detect sounds
detection <- energy_detector(files = "simulated_2.wav",  threshold = 0.5, path = tempdir())

# plot spectrogram
label_spectro(wave = simulated_2, envelope = TRUE, threshold = 0.5, detection = detection)
...
# detect sounds
detection <- energy_detector(files = "simulated_2.wav",  threshold = 0.5, min.duration = 1, path = tempdir(), hold.time = 200)
...
`smooth` works by merging the amplitude envelope 'hills' of the split signals themselves. It smooths envelopes by applying a sliding window averaging of amplitude values. It's also given in ms, which control the size of the window. A `smooth` of 350 ms can merged back split signals from our example:  

```{r, eval = TRUE}

# detect sounds
detection <- energy_detector(files = "simulated_2.wav",  threshold = 0.5, min.duration = 1, path = tempdir(), smooth = 350)

# plot spectrogram
label_spectro(wave = simulated_2, envelope = TRUE, threshold = 0.5, detection = detection, smooth = 350)
...
This last example using `ssmooth` can be used to showcase how the tunning parameters can be optimized. As explained above, to do this we need a reference table that contains the time position of the target signals. The function `optimize_energy_detector()` can be used finding the optimal parameter values. We must provide the range of parameter values that will be evaluated:

```{r}

optimize_energy_detector(reference = sim2_sel_table, files = "simulated_2.wav", threshold = 0.5, min.duration = 1, path = tempdir(), smooth = c(100, 250, 350))[, c(1, 3:6, 8:13, 18:19)]
...
This detection method is better suited for highly stereotyped signals and it's more robust to higher levels of background noise. The procedure is divided in two steps:

- Estimating the cross-correlation of template(s) along sound files (`template_correlator()`)  
- Detecting signals by applying a correlation threshold (`template_detector()`)  

We can apply this method to detect the signals in the example 'lbh1' data as follows:
```{r}

# get correlations 
correlations <- template_correlator(templates = lbh1_reference[1, , drop = FALSE], files = "lbh1.wav", path = tempdir())
...
We can explore it by plotting the spectrogram along with the detection and correlation scores:
```{r, warning=FALSE}

# plot spectrogram
label_spectro(wave = lbh1,  detection = detection, template.correlation = correlations$`lbh1.wav-10/lbh1.wav`$correlation.scores, flim = c(0, 10), threshold = 0.4)
...
The function `optimize_template_detector()` allows to evaluate the performance unde different correlation thresholds:

```{r}

# run optimization
optimization <- optimize_template_detector(template.correlations = correlations, reference = lbh1_reference, threshold = seq(0.1, 0.9, 0.05))
...
optimization <- optimize_template_detector(template.correlations = correlations, reference = lbh1_reference, threshold = c(0.01, 0.99), previous.output = optimization)
...
Several templates can be used within the same call. Here we correlate two templates on the two example sound files, one template from each sound file:
```{r}
...
```{r}
# filter
filtered <- filter_detection(labeled)
